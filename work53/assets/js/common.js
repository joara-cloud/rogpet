$(document).ready(function() {
	...
	// 문의하기
	$(document).on('click' ,'.layerpopup .email', function(e) {
		e.preventDefault();

		$('.layerpopup').addClass('is_qna');
	});

	// 레이어팝업
	$('.layer_popup .btn_close, .layer_popup .btn_cancel').click(function(e) {
		e.preventDefault();
		$('.layer_popup').hide();
	})
	...
	for (j = 1; j < ll; j++) {
		...
		c.addEventListener("click", function(e) {
			...
			if (s.options[i].innerHTML == this.innerHTML) {
				...
				document.querySelector('.select-selected').classList.add('selected')
				break;
			}
			}
			h.click();
